...
int get_info(int type, char **list)
{
  if (type > G_MAX)
    return (-1);
  switch (type) {
...
int get_windata(struct window_data *windatap)
{
...
int get_eachwin(struct window_data *windatap)
{
  static int i = 0;
...
int get_all(struct window_data *list)
{
  int i;
...
int get_client(struct window_data *list)
{
  int i;
...
int get_colrow(int *cols, int *rows)
...
int m_makewindow(int x, int y, int wide, int high)
{
  _m_ttyset();
