  ...
  int             i, j;
{
  ...
  field[i][j].caught = ! Free;	/* was != making a noop */
  return (!Free);
}
...
calculate_field()
{
	  ...
	  if (!(curorder == STAND) && !(field[i][j].changed))
	  {
	    ...
	    case BOULDER:
	      jjj = (2 * jj - j + x) % x;	/* the cell behind the
						 * boulder */
	      if (field[i][jjj].content == SPACE && ((rand() % 2) == 0) &&
		  !(field[ii][jj].dir == E))
	      {
	...
	case EATER:
	  ...
	  } else
	  {
	    ...
	    case BOULDER:
	      jjj = (2 * jj - j + x) % x;	/* the cell behind the
						 * boulder */
	      if (field[i][jjj].content == SPACE && ((rand() % 2) == 0) && !(field[ii][jj].dir == E))
	      {
		move_cell(i, jj, i, jjj);
		move_cell(i, j, i, jj);
	      } else
  ...
  blobcells = 0;
  pgc = (pgc == pgc1) ? ((rand() % 22) == 0) ? pgc2 : pgc1 :
    pgc1;
