...
#define MAXSHELL \
  127                 /* max chars read from shell at once (128 causes an error) ?! */
...
#define MAXEVENTS                                            \
  22                    /* max number of events, -4 thru 17, \
                        see event.h */
...
#define DEFAULT_FONT \
  "DEFAULT_FONT"        /* default font environ variable (full path name) */
...
#define MENU_COLOR                           \
  BUILDOP(BIT_SRC, color_map[MENU_COLOR_FG], \
      color_map[MENU_COLOR_BG]) /* menu colors */
...
#define ACTIVE_ON()                                            \
  border(active, BORDER_FAT),                                  \
      ...
      ? (do_event(EVENT_DEACTIVATED, last_active, E_MAIN),     \
            do_event(EVENT_ACTIVATED, active, E_MAIN), active) \
      : last_active

#define ACTIVE_OFF() border(active, BORDER_THIN)

#define SETMOUSEICON(x) (m_rop = x)
#define DEFAULT_MOUSE_CURSOR \
  ((active && active->cursor) ? active->cursor : &mouse_arrow)
...
#define W_NOBUCKEY                                                  \
  0x200000L                  /* Buckey keys (i.e. "Left-<char>" and \
...
#define W_LOOK (W_EXPOSE | W_BACKGROUND) /* Window ready to accept data */

#define W_STATE (W_ESCAPE | W_TEXT) /* terminal emulator states */

#define W_SAVE \
  (W_REVERSE | W_STANDOUT | W_EXPOSE | W_BACKGROUND | W_VI | W_NOINPUT | W_NOWRAP | W_SNARFLAGS | W_OVER | W_ABSCOORDS) /* savable flags */
