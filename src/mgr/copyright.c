...
/*}}}  */
/*{{{  variables*/
static BITMAP *logo[] = { &ball_1, &ball_2, &ball_3, &ball_4, &ball_5, &ball_6, &ball_7, &ball_8 };
...
static void
cordic(short *x, short *y, int scale, int count)
...
void copyright(BITMAP *where, char *password)
{
  ...
  if (at_startup) {
    ...
    g = 0;
    b = 0;
    maxi = 255; /* black for screen saving */
  }
  findcolor(screen, &ind, &r, &g, &b, &maxi);
