...
#define menu_ischoice(state) \
  ((state)->current >= 0 && (state)->current < (state)->count)
#define menu_ypos(state) \
...
#define menu_next(state)                                                        \
  (menu_exit(state) == EXIT_BOTTOM && (state)->next >= 0                        \
          ? (state)->next                                                       \
          : (menu_ischoice(state) ? (state)->action[(state)->current].next_menu \
                                  : -1))
...
struct menu_state {
  ...
  short bar_sizex,            /* size of one menu item */
      bar_sizey, x_pos;       /* x position relative to inside */
