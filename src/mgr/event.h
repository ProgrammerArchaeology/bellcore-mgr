...
#define E_WINSIZE 'X'   /* get window size */
...
#define E_LIST_BUTTON "%pPrRbBlTtnwXSM" /* valid esc's for buttons */
#define E_LIST_UP "%pPMwnX"             /* valid esc's for button ups */
...
#define EVENT_CLEAR_MASK(win, event) \
  ((win->event_mask) &= ~(1L << GET_EVENT(event)))
#define IS_EVENT(win, event) ((win->event_mask) & (1L << GET_EVENT(event)))
