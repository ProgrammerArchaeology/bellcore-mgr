    ...
    )
{
  ...
  if (menu_value(state) && *menu_value(state) && (current = (struct menu_result *)
...
int exit_code;                  /* valid exit codes */
{
  ...
  while (!done) {
    ...
    dbgprintf('m', (stderr, "  from user ..."));
    fflush(stderr);

    /* do auto right menus */

    if (state->flags & MENU_PAGE && exit_code & EXIT_BOTTOM && first && state->current >= state->count) {
      first = 0;
